...
technicalities of the following argument. The idea is to use trees to model the
interaction between input and output symbols. The directions model inputs 
while symbols represent outputs. Similar to the arguments of the proof of 
Theorem \ref{thm:POMDPequivWDTA} a tree can therefore be understood as strategy
and we want to design a \ac{PTA} such that a tree is only accepted if all paths
are part of $\mathcal{L}_{\phi}$. This can be easily modelled (cp. for example 
\cite{PathLang}) with
\begin{definition}[Synthesis \ac{PTA}]
  ...
  \begin{equation*}
    ...
    \text{ with }
    \Delta = \set{
      \tuple{q, j, (\delta(q, \tuple{j, d}))_{i\in I}}
      \text{ for all }q\in Q,j\in J
    }.
  \end{equation*}
\end{definition}
Notably, $\mathcal{A}$ executes $\mathcal{P}$ on every path and therefore, if we
choose $\mathcal{P}$ as the \ac{PTA} which precisely accepts 
$\mathcal{L}_{\phi}$, we deduce that $\mathcal{A}$ accepts only those strategies 
$t$ which satisfy $\phi$. Hence, deciding the emptiness concludes the argument 
to formulate a synthesis algorithm for $\omega$-regular specifications $\phi$ 
and gives
\begin{theorem}
