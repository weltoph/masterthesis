...
\end{theorem}
Since the cylindric sets do satisfy the conditions on $\mathcal{E}$ we obtain 
(as illustrated in Figure \ref{fig:pbaruns}) a probability space 
\begin{equation*}
  \tuple{Q^{\omega}, \mathcal{B}(Q), \mu_{\alpha}}.
\end{equation*}

\begin{drawing}
  \caption{Illustration of runs $p, q, b, k \in Q^{\omega}$ of a \ac{PBA} 
  $\mathcal{P}$ on a word $\alpha = \alpha_{1}\alpha_{2}\dots$ and the induced
