...
\begin{proof}
  ...
  \end{lemma}
  \begin{proof}
    We introduce here a common theorem regarding probability spaces.
    \begin{theorem}[Borel-Cantelli Lemma]
      ...
      all $i \neq j$ and $i,j > 0$ is called \emph{independence} of all $A_{i}$
      for $i > 0$.
    ...
    that is possible after a sequence $\tuple{b^{n}a}^{k}$ (for $k = 1, 2, 
    \dots$) it is probable to read another accepting state within the next 
    $b^{n}a$ sequence. Hence, fixing $A_{k}$ as the observation that in the 
    $k$-th reading of $b^{n}a$ from every state at least once a final state is 
    visited allows to use Theorem \ref{thm:BorelCantelli} for the sequence 
    $\tuple{A_{k}}_{k > 0}$ and yields an almost-sure probability for reading
    infinitely many states in $F$ for runs of $\mathcal{P}$.
    \fxwarning{is now proof, make bueno}
  \end{proof}
\end{proof}
