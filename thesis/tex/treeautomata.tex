...
$\mathcal{G}(\mathcal{A}, t)$ with exchanged roles of \eve{}
and \adam{}. \adam{} pro-actively chooses his answers and \eve{} choice of one
direction corresponds to an implicit choice of a transition, namely the one to 
which \adam{} answers by this movement. Nevertheless the resulting games can be
found as plays in $\mathcal{G}(\mathcal{A}, t)$. We construct for a given 
\ac{APTA}
\begin{equation*}
...
set of accepted paths. Therefore $\mathcal{C}$ is the dual automaton to 
$\mathcal{A}$ implying
\begin{proposition}
...
\ac{APTA}. This is due to the fact that the choices of \automaton{} must be 
consistent with an underlying tree (cp. Example \ref{ex:pdaaspomdp}). We 
...
\begin{example}
  ...
  \begin{equation*}
    \set{
      \tuple{q_{\sigma}, \sigma, \delta_{\set{\tuple{q_{\sigma, d}}}}},
      \tuple{q_{\sigma}, \overline{\sigma}, \delta_{\set{\tuple{\bot, 0}, 
  ...
  \begin{equation*}
    \set{
      \tuple{q_{0}, \sigma, \delta_{\set{\tuple{q_{a}, 0}, \tuple{q_{b}, 0}}}}:
        \text{for }\sigma\in\set{a, b}
      }
  \end{equation*}
  as part of $\Delta$. Naturally, there is \emph{no} possible accepted tree for 
  $\mathcal{M}$ since $t(0)$ is either $a$ or $b$ but either choice yields one
  ...
  \ref{def:treeemptinessgame}) \automaton{} still can win by constructing 
  different trees depending on the fact if $q_{a}$ or $q_{b}$ is chosen by 
  \pathfinder{} in the intial step. For \acp{PBA} we addressed this problem 
  with partial observability and we observe in the following the necessity of
  this for alternating tree automata which incorporate a weighting for their 
  paths.
