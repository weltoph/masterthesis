...
\begin{drawing}
  \caption{
    ...
    $t$ with $t(u) = a$ if and only if $u = 00$. Nodes of player $0$ are marked
    as circles while player $1$ plays on rectangles. Every triangle is 
    associated with one word $u\in\set{0,1}^{*}$ and labeled with $t(u)$. 
    Player $0$ wins by forcing the game to the node $\tuple{q_{s}, 00}$ where
    $q_{s}$ is transformed to the accepted sink $q_{a}$.
  }
...
\begin{proof}
  ...
  As mentioned before we consider player $1$ to choose a path through the run 
  and checking this path for its acceptance. Thus, given a strategy 
  $f_{0}$ associated with a run $r$ we obtain for an additional riposte $f_{1}$ 
  of player $1$ one element in $M_{r}$ and for one element in $M_{r}$ one 
  strategy for player $1$. We fix pairs $f_{1}$ and $\alpha = 
  ...
  \end{equation*}
  The combination of two strategies $f_{0}, f_{1}$ gives a play 
  $\beta = \tuple{p_{0},\epsilon}\tuple{\delta_{A_{0}},\epsilon}\tuple{p_{1}, 
  u_{1}}\dots\in V^{\omega}$ in $\mathcal{G}(\mathcal{A}, t)$. Furthermore,
