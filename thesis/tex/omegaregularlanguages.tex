...
established. We recall some of the central results and provide proofs which 
allow us to define auxiliary structures in a simple context. These structures
are later used in more elaborate constructions.
Our arguments follow, in some condensed and
...
\begin{proof}
  ...
  $\parity'(q) = \parity(q) + 1$ (denoted by $\parity' = \parity + 1$) and 
  keeping the original structure we exchange non-accepting and accepting runs 
  and therefore obtain a structural equivalent \ac{DPA} that precisely accepts 
  the complement of the original automaton.
\end{proof}

At last, we want to shortly discuss that deciding emptiness for 
...
at least one $f\in F$ has to occur infinitely often in $\pi$.
It is therefore
