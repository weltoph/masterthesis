...
\begin{definition}[Tree automaton]
  ...
  all $d\in D$ and $\sigma\in\Sigma$. We define a tree automaton
  \begin{equation*}
...
\begin{definition}[Alternating Tree Automata]
  ...
  Parity-conditions. 
\end{definition}
We introduce according acronyms, namely \ac{ABTA}, \ac{ARTA}, \ac{AMTA} and
\ac{APTA} respectively. By using the same argument of using \acp{LAR} as for
Theorem \ref{thm:omegaregularexp} and Theorem \ref{thm:treeautoequiv} we obtain
\begin{theorem}
  \cite[Consequence of Theorem 1.2]{SimAltTreeAuto}
...
\begin{drawing}
  ...
  alternating tree automata. If we consider this as the runs of a union
  (intersection) positively (almost-surely) accepting \ac{PBA} both runs are
  tuned down by the factor $\frac{1}{2}$.}
...
\begin{definition}[Graph Games]
  \begin{description}
    ...
    \item [Strategy] For $\sigma\in\set{0,1}$ we define a strategy
      ...
      one player (\eve{}) if it belongs to $\Acc$. Notably it holds that two
      strategies $f_{0}, f_{1}$ induce a play $v_{0}v_{1}\dots$ by fixing for
      any $v\in V$ the term
      \begin{equation*}
        \sigma(v) = \begin{cases}
          0&\text{if }v\in V_{0}\\
...
determinacy by exploring all of \eve{}'s option since a winning strategy of
\adam{} produces for all choices of \eve{} a winning play, hence choosing the
set of answers that are initial for winning plays allows to encode \adam{}'s
winning strategy. We construct for a given 
\ac{APTA}
\begin{equation*}
...
$\mathcal{A}$ implying
\begin{proposition}
  \cite[Complementation Theorem]{AltTreeAuto}
