...
\begin{proof}
  ...
  $\mathcal{L}_{\exists a}$. For this we prove the following lemma:
  \begin{lemma}
    For any run $r$ of a \acp{WDTA} $\mathcal{A}$ with a Parity-condition 
  ...
  $r_{0}(\epsilon) = r_{1}(\epsilon) = r_{b}(\epsilon)$. We can fix
  into $T_{0}$ and $T_{1}$ with $T_{i} = \set{\tuple{q, i}:\text{for }q\in Q}$ 
  for $i = 1,2$. We can state that $\cyl(T_{0})\cup\cyl(T_{1})$ parititons 
  $\tuple{Q\times D}^{\omega}$. We can fix $S_{i} = \prolong(T_{i})$ and 
  observe that $r_{b}(u) = r_{i}(u)$ for all $u\in S_{i}$ ($i = 1,2$). Hence,
  we see that $\mu_{r_{u}}$ coincides with $\mu_{r_{i_{u}}}$ for $u\in S_{i}$ 
  ($i = 1,2$) and therefore with Lemma \ref{lem:subrunsum} we obtain
  \begin{align*}
    \mu_{r_{b}}(\Acc\interval{Q}(\parity)) 
    &= \sum\limits_{q\in Q}\mu_{r_{0}}(\cyl(\tuple{q, 1}))\cdot
      \mu_{r_{\tuple{q, 1}}}(\Acc\interval{Q}(\parity))\\ 
    &+ \sum\limits_{q\in Q}\mu_{r_{1}}(\cyl(\tuple{q, 0}))\cdot
      \mu_{r_{\tuple{q, 0}}}(\Acc\interval{Q}(\parity))
  \end{align*}
  and since $r_{0}$ and $r_{1}$ are almost-sure accepting it holds that
  $\mu_{r_{\tuple{q, 0}}}(\Acc\interval{Q}(\parity)) = 1$ for all $q\in Q$ such
  that $\mu_{r}(\cyl(q, 0)) > 0$ and analogously
  $\mu_{r_{\tuple{q, 1}}}(\Acc\interval{Q}(\parity)) = 1$ for all $q\in Q$ such
  that $\mu_{r}(\cyl(q, 1)) > 0$ and therefore 
  $\mu_{r_{b}}(\Acc\interval{Q}(\parity)) = 1$. This contradicts that 
  $\mathcal{A}$ precisely accepts $\mathcal{L}_{\exists a}$ since $t_{b}$ is
  accepted.
\end{proof}
