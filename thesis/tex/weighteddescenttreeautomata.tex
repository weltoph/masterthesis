...
player (i.e.  the strategy) are which transition is used at which position of 
the run. Hence, the player effectively constructs with his choices the tree $t$ 
by providing for every situation the corresponding letter. We define
\begin{definition}[Run Building Game]
  For a choiceless \ac{WDTA} 
  $\mathcal{A} = \tuple{Q, q_{0}, D, \Sigma, \Delta, \parity}$
  we define the set of actions for the player as $\Sigma$.
  Thus, any action in $A$ corresponds to one tuple $\tuple{G_{0},\dots,G_{n}}$
  such that every generator is given for one state and every
