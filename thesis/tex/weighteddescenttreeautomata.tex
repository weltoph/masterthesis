...
\begin{proof}
  ...
  recognizeable by a \ac{WDTA} and the argument is conceptually simple. We 
  consider a tree with precisely one path that does not contain an $a$ but all
  other paths conceal their $a$ farther and farther which induces the necessity
  to carry weight in a certain width. This allows to argue that the measure of
  the one non-accepting path is negligeable. We define 
  $t:\set{0,1}^{*}\rightarrow\set{a,b}$ with $t(u) = a$ if and only if 
  $u\in \set{0,1}^{*}1$.
...
\begin{proof}
  ...
  measure on $\mathcal{B}(Q)$ as the stochastic process of $\mathcal{P}$. Since
  the acceptance conditions also coincide we can ensure that the automata are
  equivalent regarding positive as well as almost-sure acceptance.
...
any run of a \ac{WDTA} effectively is a unrollment of a transition system which
is induced by a tree $t$. This behavior appears very similar to the concept of
\acp{MDP} where the strategy of the actor unrolls the transition system. 
Indeed, we establish a very deep connection between \acp{POMDP}. Initially, we
show that \acp{WDTA} can be used to model \acp{POMDP} where trees are 
interpreted as strategies. Furthermore, we use the mechanism of \acp{POMDP} to
establish decideability results for algorithmic questions about \acp{WDTA},
