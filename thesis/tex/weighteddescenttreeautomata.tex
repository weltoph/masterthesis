...
argument here is that by an inductive argument there is always at most one 
state with weight at every position in the input tree. Especially this entails 
that the corresponding equivalent \ac{POMDP} does not require the partial 
observability for the interchangeability of the associated strategy with a 
tree. Considering only the associated \ac{MDP} without the observation classes 
allows to use Theorem \ref{thm:mdpstratsynthesis} to obtain a decision 
procedure for the emptiness problem.
\begin{corollary}[Emptiness Almost-Sure Uni-Directional WDTA]
  ...
  acceptance measure of a Parity-condition can be decided in
  polynomial time.
\end{corollary}
