...
conditions (as suggested but not proven in \cite[page 24:9]{RandAutoInfTrees} 
...
union, intersection and negation. Unsuprisingly, the non-determinism of the 
model induces closure under union for certain classes in a straightforward 
manner (as suggested by \cite{RandAutoInfTrees}). 
...
\begin{proof}
  ...
  \begin{equation*}
    \mathcal{B} = \tuple{Q_{1}\cup Q_{2}\uplus\set{q^{i}}, q^{i}, D, \Sigma,
    \Delta_{1}\cup\Delta_{2}\cup\Delta^{i},\parity_{\cup}}
  \end{equation*}
  where $q^{i}$ is a new state and
  \begin{equation*}
    \Delta^{i} = \set{
      \tuple{q^{i},\sigma,G\interval{}}:\text{for every }\tuple{q_{0},\sigma,G}
      \in\Delta_{1}\text{ and }\tuple{p_{0},\sigma,G}\in\Delta_{2}
    }
  \end{equation*}
  is the union of all initial transitions from $\Delta_{1}$ and $\Delta_{2}$
  and
  \begin{equation*}
    \parity_{\cup}(q) = \begin{cases}
      \parity_{1}&\text{if }q\in Q\\
      \parity_{2}&\text{if }q\in P\\
      \min\tuple{\parity_{1}(Q)\cup\parity_{2}(P)}.&\text{if }q = q^{i}.
    \end{cases}
  \end{equation*}
  The initial decision in $q^{i}$ leads to one generator
