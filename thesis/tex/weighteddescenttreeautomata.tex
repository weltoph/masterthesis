...
uniformly distributed \acp{WDTA}. For \acp{PBA} we already established with
Lemma \ref{lem:measureinpba} and its consequences that the choice of the 
transition probabilities does matter. In this context we discuss some results 
from \cite{RandAutoInfTrees}. Firstly, we introduce the following example:
\begin{example}
  ...
  \begin{equation*}
    \Delta = \set{
      \tuple{q_{a}, \sigma, G_{\sigma}}, \tuple{q_{b},\sigma, G_{\sigma}}:
...
\begin{proposition}
  \cite[Proposition 11]{AutoRandInfTrees}
