...
\begin{proof}
  ...
  $\alpha_{r}\in\tuple{Q\times \set{0}}^{\omega}$ (cp. 
  \cite[Example 1.30]{Klenke} or \cite{POSG}). This means that
  \begin{equation*}
    \mu_{r}(A) = \begin{cases}
...
automata (Theorem \ref{thm:simulation}) does not translate to almost-surely
accepting \acp{WDTA}. This result is disappointing but not surprising since
we established the correspondance between alternation and partial observability
and fundamental differences in terms of algorithmic properties between 
\acp{MDP} and \acp{POMDP} are well established.
