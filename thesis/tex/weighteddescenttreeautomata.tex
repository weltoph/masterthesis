...
\end{equation*}
These are those paths that actually carry weight. Is is easy to see that only
those elements of $r$ in $W_{r}$ actually matter for the weighting of a run, 
hence we state
\begin{lemma}
...
\begin{proof}
  ...
  $\mu_{r_{i}}(\cyl(a)) = 0$ for $i = 1,2$. We therefore deduce that
...
arguments for \acp{PBA} or tree automata. We examine these properties 
selectively, predominantly focusing on almost-surely accepting models because 
they prove most relevant for the examined use-cases later on. 
...
which is similar to \acp{PBA} (cp. Lemma \ref{lem:measureinpba}). But in both 
arguments the examined models are deterministic in the choice of their 
transitions. Naturally, this rises questions about the incluence to use 
non-determinism in probabilistic models. As we see later the inclusion of 
non-determinism is a significant source of complexity of the model
(cp. \cite[Remark 12]{RandAutoInfTrees}).
...
common product construction to yield an intersection operator.
\begin{proposition}
  \cite[Proposition 14]{RandAutoInfTrees}
...
complete probability mass. Therefore, the acceptance of the \acp{WDTA} is 
defined by the satisfaction of the one path that carries the complete 
probability mass. The acceptance of this path is defined via a $\omega$-regular 
acceptance set. These observations lead naturally to the equivalence of these 
automata models, i.e.
\begin{theorem}
...
\begin{proof}
  ...
  induces a Dirac measure\footnote{cp. \cite[Example 1.30]{Klenke} or 
  \cite{POSG}} $\mu_{r}$ on $\mathcal{B}(Q\times D)$ for one path 
...
\begin{lemma}
  ...
  respectively.
\end{lemma} 
Moreover, we obtain the integrability of $g$ by its boundness and therefore 
...
system.  Indeed, we establish a very deep connection between \acp{POMDP} and 
\acp{WDTA}. Initially, we show that \acp{WDTA} can be used to model \acp{POMDP} 
where trees are interpreted as strategies. Furthermore, we use the mechanism of 
\acp{POMDP} to establish decideability results for algorithmic questions about 
\acp{WDTA}, e.g. deciding emptiness, or respectively deduce undecideability 
results. This allows us to reveal intriguing connections between \acp{WDTA}
and \acp{POMDP}; for example the fact that alternation in tree automata models 
...
strategy. The choicelessness of the \ac{WDTA} ensures to obtain an 
interchangability of the strategies and trees since the choices of a strategy
immediately determine the transition probabilities while general \acp{WDTA} 
allow for a \enquote{second} decision once the letter is chosen, namely which
transition to take. Nevertheless, this allows to formulate the emptiness 
problem of a choiceless \ac{WDTA} as computation of a strategy for \acp{POMDP}. 
The choices of \eve{} (i.e.  the strategy) are which letter is used at which 
position of the tree. Hence, the player effectively constructs with his choices 
the tree $t$ by providing for every situation the corresponding letter. We 
define analogously to \cite[Section 3.5 Emptiness Problem]{RandAutoInfTrees}
...
\end{proof}
Regarding the choicelessness requirement on the modelled \ac{WDTA} we expand in
...
algorithmically easier to approach while maintaining an interesting measure of
expressibility.

\subsection{Consequences}
Modelling probabilistic word automata entails immediately undecideability 
results. Namely, Theorem \ref{thm:pbaaswdta} allows to deduce the 
undecideability of the emptiness problem even for choiceless \acp{WDTA} with 
positive acceptance of an associated Büchi-condition (and therefore also for 
Rabin-, Parity- and Muller-conditions) and by Corollary 
\ref{cor:emptinessstrongalmostsureprob} also almost-sure acceptance of more 
expressive acceptance conditions (alternatively one can use the translation 
from \acp{POMDP} to \acp{WDTA} as equivalent argument)
\begin{corollary}
...
\begin{corollary}[Emptiness Almost-Sure Uni-Directional WDTA]
  ...
  acceptance measure of a Büchi- or Parity-condition can be decided in time 
  polynomial in $\size{Q}\cdot\size{D}$.
\end{corollary}
\begin{proof}
...
accepting \acp{WDTA} which is disappointing but not surprising since
we established the correspondance between alternation and partial
observability. Specifically, by the observation that we can compute strategies 
for \acp{MDP} with Parity-conditions efficiently while this is undecideable for
\acp{POMDP} with Parity-conditions already establishes that there is no 
effective transformation from multi-directional to uni-directional \acp{WDTA}.
...
\begin{proof}
  ...
  $\mathcal{L}_{\exists a}$. For this we prove some lemmas which provide
  ...
  \begin{proof}
    ...
    \end{equation*}
  \end{proof}
  This allows to deduce the following observation which is also observed in the
  ...
  an almost-sure acceptance of a Parity-condition to accept 
...
The classes of languages accepted by \acp{WDTA} or non-weighted tree automata
respectively are also incomparable:
\begin{theorem}
  The class of languages recognizable with almost-sure accepting \acp{WDTA} and 
  the class of languages recognizable with \acp{PTA} are incomparable.
\end{theorem}
\begin{proof}
  First, we observe that there is a language that is recognizable by a 
  \ac{WDTA} while it is not recognizable by a \ac{PTA}. Consider the language
  of trees over directions $\set{0,1}$ and the alphabet $\set{a,b}$ such that
  under uniform weighting of all paths the set of paths that contain infinitely 
  many $a$ has measure $1$. This language can be accepted by a choiceless
  uniformly distributed \ac{WDTA} $\mathcal{A}$ with two states $q_{a}, q_{b}$, 
  where the uniform distribution is defined by the blueprint 
  $B(0) = B(1) = \frac{1}{2}$ and both states send down $q_{\sigma}$ in both
  directions when they read $\sigma\in\set{a,b}$. The associated 
  Büchi-condition $F = \set{q_{a}}$ ensures that $\mathcal{A}$ accepts 
  precisely the correct language (cp. \cite[Example 7]{RandAutoInfTrees}). 
  Moreover, it can be shown that there is no \ac{PTA} that accepts this 
  language leading to
  \begin{lemma}
    \cite[Theorem 21]{RandAutoInfTrees}
    There is a language that is recognizable by a choiceless uniformly 
    distributed \ac{WDTA} but not by any \ac{PTA}.
  \end{lemma}

  On the other hand, we consider the language $\mathcal{L}$ of trees over 
  directions $\set{0,1}$ and the alphabet $\set{a, b}$ such that every path 
  contains an $a$. This language can be accepted by a \ac{BTA} with two 
  states $q_{a}, q_{b}$. Starting in $q_{b}$ it sends down $q_{b}$ states in 
  all directions if a $b$ is read. If an $a$ is read $q_{b}$ transform to the 
  sink state $q_{a}$ which is also the accepting state. Hence, if all paths 
  contain an $a$ eventually all $q_{b}$ transform and the run is accepted. If 
  there is a path that does not contain an $a$ then on this path only $q_{b}$ 
  are reproduced in the run rendering it not accepted.
 
  We claim that this language cannot be accepted by a \ac{WDTA} by the 
  following argument: For the sake of provoking a contradiction assume there is 
  a \ac{WDTA} $\mathcal{A}$ with associated Parity-condition $\parity$ 
  almost-surely accepting this language. We introduce the notion of
  \emph{configurations} $C\subseteq Q$. We call $C$ the configuration
  of $u = d_{1}\dots d_{n}\in D^{*}$ in a run $r$ if for all 
  $q_{1},\dots, q_{n}\in Q^{n}$ holds $q_{n}\in C$ if and 
  only if $\mu_{r}(\cyl(\tuple{q_{1}, d_{1}}\dots\tuple{q_{n}, d_{n}})) > 0$.
  W.l.o.g. we can assume that $\mathcal{A}$ only reaches non-empty 
  configurations $C = \set{q_{1}, \dots, q_{n}}$ such that there is a tree $t$
  on which $\mathcal{A}$ may produce an almost-surely accepting run $r$ if 
  $q_{i}$ is the initial state (for all $1\leq i\leq n$). This is rooted in the
  fact that every partial tree can be extended such that it is part of 
  $\mathcal{L}$. It is therefore not possible that every run of $\mathcal{A}$ 
  produces after finitely many steps configurations that are \enquote{dead} (in
  the sense of the impossibility to accept anymore). Hence we consider only 
  those runs such that every reachable configuration can be completed to be 
  almost-surely accepting. For every $u = d_{1}\dots d_{n}\in D^{*}$ we introduce 
  the notion of weight in a run $r$
  \begin{equation*}
    w_{r}(d_{1}\dots d_{n}) = \sum_{q_{1}\dots q_{n}\in Q^{n}}\mu_{r}(\cyl(
      \tuple{q_{1}, d_{1}}\dots\tuple{q_{n}, d_{n}})).
  \end{equation*}
  We argue that for the tree $t_{b}$ which only contains $b$ any run $r$ of
  $\mathcal{A}$ must satisfy $w_{r}(u) > 0$ for all $u\in D^{*}$. If this is
  not the case the subtree rooted at $u$ is not longer explored, producing for 
  all other $v\in D^{\size{u}}\setminus\set{u}$ starting in $v$ a subtree such 
  that the configuration in $v$ produces an almost-surely accepting run but 
  keeping up to $u$ and the subtree of $u$ $a$-free yields an almost-surely 
  accepting run on a tree which is not part of $\mathcal{L}$. On the other 
  hand, there has to be increasing sequence $u_{0}\sqsubseteq u_{1}\sqsubseteq 
  u_{2}\sqsubseteq\dots$ such that $\tuple{w_{r}(u_{i})}_{i > 0}$ converges to
  $0$. Keeping for this path only $b$ but choosing for all other subtrees trees
  which render the corresponding configurations almost-surely accepting also 
  yields an almost-accepting run since the run along these $u_{i}$ (for 
  increasing $i$ is negligeable) for a tree which is not part of $\mathcal{L}$. 
  Concludingly, we obtain that $\mathcal{L}$ cannot be accepted by any 
  \ac{WDTA}.
\end{proof}
