...
\begin{definition}[Probabilistic Weighted Automata]
  ...
  possible transitions for one tuple $q\in Q,\sigma\in\Sigma$ are defined as a 
  non-empty set
  \begin{equation*}
    \Tau_{q}^{\sigma} = \set{
      \tau_{1},\dots,\tau_{n}
  ...
  \end{equation*}
  where $\tau_{i}:D\rightarrow Q$ for $1\leq i\leq n$ and $\Tau$ is the union 
  of $\Tau_{q}^{\sigma}$ for all $q\in Q$, $\sigma\in\Sigma$. The transition 
  are \enquote{chosen} by a function $\delta:Q\times\Sigma\times\Tau$ such that
  \begin{equation*}
    \delta(q,\sigma,\tau) > 0\text{ implies }\tau\in\Tau_{q}^{\sigma}
  \end{equation*}
  and every $\delta(q,\sigma,\cdot)$ is a probability distribution over 
  $\Tau_{q}^{\sigma}$. Concludingly, we obtain \ac{PWA}
  \begin{equation*}
    \mathcal{A} = \tuple{Q, q_{0}, D, \Sigma, B, \Tau, \delta, T}.
  \end{equation*}
\end{definition}
...
define the semantic of one run by constructing for a pair 
$q\in Q$ and $\sigma\in\Sigma$ with 
$\Tau_{q}^{\sigma} = \set{tau_{1}\dots\tau_{n}}$ a set of transitions
$\set{\tuple{q,\sigma,G_{1}},\dots,\tuple{q,\sigma,G_{n}}}$ such that
\begin{equation*}
  G_{i}(p,b) = \begin{cases}
    B(b)&\text{if }\tau_{i}(b) = p,\\
    0&\text{otherwise}.
  \end{cases}
\end{equation*}
Although sufficient, this semantic feels due to its complexity rather bloated
...
$\tau\in\Tau_{q}^{\sigma}$ such that $r(w\cdot d) = \tau(d)$ for all $d\in D$. 
As seen before, the distribution $B$ induces a measure $\mu_{B}$ on 
the Borel-algebra $\mathcal{B}(D)$. For a run $r$ we can collect the set of 
accepting paths regarding the acceptance condition $T$ as 
$\Acc_{r}(T) = \set{\alpha\in D^{\omega}\mid r(\alpha)\in T}$.
We consider such a run $r$ almost-surely accepting if 
$\mu_{B}(\Acc_{r}(T))=1$ and positively accepting if $\mu_{B}(\Acc_{r}(T))>0$.
...
\begin{definition}[Prefix-Tree]
  ...
  prefix-tree if for every $w\in T$ and every finite prefix $v\sqsubseteq w$ 
  holds $v\in T$. Additionally, we call a prefix-tree $T$ \emph{proper} if for 
  any $w\in T$ exists $d\in D$ such that $w\cdot d\in T$ also $w\cdot d'\in T$ 
  for all other $d'\in D$ holds. We define the \enquote{inner} nodes of $T$ as
