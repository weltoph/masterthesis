...
\begin{definition}[Probabilistic Weighted Automata]
  ...
  Concludingly, we obtain \ac{PWA}
  \begin{equation*}
    \mathcal{A} = \tuple{Q, q_{0}, D, \Sigma, B, \Delta, \delta, T}.
  \end{equation*}
\end{definition}
...
$\mu_{B}$ on the Borel-algebra $\mathcal{B}(D)$. For a run $r$ we can collect 
the set of accepting paths regarding the acceptance condition $T$ as 
$\Acc_{r}(T) = \set{\alpha\in D^{\omega}\mid r(\alpha)\in T}$. We consider such 
a run $r$ almost-surely accepting if $\mu_{B}(\Acc_{r}(T))=1$ and positively 
accepting if $\mu_{B}(\Acc_{r}(T))>0$.
