...
the same argument as for Lemma \ref{lem:measureabilityAcceptance} or
...
In the following, we focus on Parity-conditions and motivate this by the
...
We distinguish certain structural properties on \acp{WDTA} allowing us to
...
\end{proposition}
% The natural question arises whether this closure persists for the complete
% class of uniformly distributed \acp{WDTA}. Although we do not present immediate
% results regarding this question, we discuss interesting observations. Namely,
% we examine the relevance of the local probabilities regarding the resulting
% measure. For \acp{PBA} we already established with Lemma \ref{lem:measureinpba}
% and its consequences that the choice of the transition probabilities matters.
% In this context we discuss some results from \cite{RandAutoInfTrees}. Firstly,
...
% transitions. Naturally, this raises questions about the influence to use 
...
In the following, we obtain closure under intersection for \acp{WDTA} with
...
each. Nevertheless, both these runs individually need an acceptance measure of
$1$ to render the new run almost-surely accepted.
\begin{proposition}
...
define the semantics of one run by defining for every pair $q\in Q$ and 
...
\begin{equation*}
  \mu_{t}(\compRuns(r)) = \prod\limits_{w\in\inner(T)}
    \delta(r(u),t(u),\tau_{u}).
\end{equation*}
This, again using Theorem \ref{thm:measureext}, allows us to obtain a 
...
Firstly, to ease the following argument we restrict our attention to partial 
...
deduce by Theorem \ref{thm:tonelli} the measureability
\begin{equation*}
...
Theorem \ref{thm:measurableintegrabl} and therefore deduce the measureability
of $\set{r\in\Runs\mid\mu_{B}(\Acc(r)) = 1 }$ as $g^{-1}(1)$. This lays the 
basis for 
\begin{proposition}
...
\begin{corollary}[Emptiness Almost-Sure Uni-Directional WDTA]
  ...
  positive acceptance measure of a Büchi- or Parity-condition can be decided.
...
does not translate to choiceless \acp{WDTA}. Firstly, we derive a few helpful
lemmas for reasoning about runs for \acp{WDTA}, i.e.
\begin{lemma}
...
Using these insights allows us to deduce
\begin{proposition}
...
\begin{proof}
  ...
  Parity-condition to accept $\mathcal{L}_{\exists a}$. The fix the unique runs 
...
\begin{drawing}
  \caption{
    ...
    inclusion symbols are used to illustrate increasing of expressiveness from 
  ...
  \label{fig:wdtamap}
  \begin{center}
    \includegraphics{tikz/wdtamap.pdf}
  \end{center}
\end{drawing}
