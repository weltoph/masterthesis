...
\acuse{DPA}
\fxfatal{find reference for following argument}
\begin{example}
...
states to more complex structures, namely trees:
\begin{definition}[Tree]
  For a finite set of directions $D$ and a finite alphabet $\Sigma$ we define
  a $D/\Sigma$ tree $t:D^{*}\rightarrow\Sigma$. Normally, since $D$ and
  $\Sigma$ are clear from the context they are omitted in the definition. Let
  $\mathcal{T}_{D}^{\Sigma}$ denote the set of all $D/\Sigma$ trees.
\end{definition}
A classic tree automaton produces a tree of states which is attached to the
input tree.
...
state while others may not receive a single one. This concept is also used to
define automata that run over transition systems, e.g.
...
of the lattice $\mathcal{L}(D\times Q)$ generated by elements in $D\times Q$
