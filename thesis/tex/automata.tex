...
\begin{definition}[Markov Decision Process]
  ...
  \end{description}
\end{definition}
We examine the set of cylinders of a \ac{MDP} $\mathcal{A}$ and for a given
strategy $\varphi_{\mathcal{A}}$ we obtain an induced probability measure for
the cylinders as follows:\fxfatal{still only a pre-measure}
\begin{equation}
...
a measure. In this fashion we obtained a pre-measure (induced by a strategy\fxfatal{show $\sigma$-additivity of pre-measure}
$\varphi$) on the ring of cylinders (cp. \cite[Chapter 1 and Chapter 3]{Bauer}).
