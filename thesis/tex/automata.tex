...
\begin{definition}[Probabilistic Weighted Automata]
  ...
  set of states $Q$. The transitions are captured by $\Delta$ which maps every
  combination of $q\in Q$ and $\sigma\in\Sigma$ to a probability distribution
  of one fixed clause $C_{q}^{\sigma}$. Furthermore, we introduce an
  $\omega$-regular target set $T\subseteq Q^{\omega}$ and pick one $q_{0}\in Q$
  to combine all these elements to a \ac{PWA}
  \begin{equation*}
    \mathcal{A} = \tuple{Q, q_{0}, D, \Sigma, \Delta, T}.
  \end{equation*}
\end{definition}
In order to define a probability distribution we introduce a $\sigma$-algebra
which is induced by partial runs, i.e. finite beginnings of runs. Therefore, we
define a finite tree as a prefix-closed subset of finite words over directions,
hence
\begin{definition}[Finite Tree]
  ...
  $v\in T$.
\end{definition}
Fixing a set of directions $D$ and an alphabet $\Sigma$ and a \ac{PWA}
$\mathcal{A}$ over $D$ and $\Sigma$. Let $t:D\rightarrow\Sigma$ be a tree and
depending on $t$ we can define all partial runs of $\mathcal{A}$ on $t$ as
follows: analogously to \acp{WDTA} a run $r$ is a tree over the set of
directions $Q\times D$. A partial run of $\mathcal{A}$ on $t$ can be obtained
by fixing a prefix-tree $T$ and examing
$r: T\rightarrow \mathcal{G}_{\mathcal{A}}$ which needs to be consistent with
$t$, i.e. for every $p = \tuple{q_{0},d_{0}}\dots\tuple{q_{n},d_{n}}\in T$ it
holds that $r(p)$ is part of the clause $C_{q_{n}}^{t(d_{0}\dots d_{n})}$.
Considering such a partial run $r$ we can define the set of runs $r'$ of
$\mathcal{A}$ on $t$ such that these runs agree on $T$ with $r$, i.e. for all
$w\in T$ holds $r(w) = r'(w)$, as $\compRuns(r)$. Analogously, to the concept
of cylinders over a set we can capture a $\sigma$-algebra
$\mathcal{R}_{\mathcal{A}}$ over runs by using the sets $\compRuns(r)$ for
every partial run $r$ as generating sets. For any partial run $r$ over the
prefix-tree $T$ (and thus for the set of compatible runs with $r$) there is an
associated probability with
\begin{equation*}
  \mu_{t}(\compRuns(r)) = \prod\limits_{
    w = \tuple{q_{0},d_{0}}\dots\tuple{q_{n},d_{n}}\in T}
  \Delta(q_{n},t(d_{0}\dots d_{n}))(r(w)).
\end{equation*}
...
\begin{enumerate}
  \item without regard for the weighting of the generators and interpreting the
    support of a generator as a clause for an alternating tree automaton the
    semantic can be defined by the measure of $\mu_{t}$ of those runs that
    satisfy a Büchi condition,
...
individual runs that are almost-surely accepting. Thus, we deem a tree accepted
if almost-all runs do satisfy the almost-sure acceptance of the objective $T$.
We therefore fix the set of all viable runs on $t$ as $\Runs$ and the set of
all possible paths in a run $\Paths = \tuple{Q\times D}^{\omega}$ and define
for an automaton $\mathcal{A}$ the following function within the product
...
\begin{equation*}
  f_{\mathcal{A}}(r, p) = \begin{cases}
    1&\text{ if the state component of }p\text{ satisfies the condition }T\\
    0&\text{ otherwise.}
  \end{cases}
\end{equation*}
