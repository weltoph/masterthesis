...
Acceptance. Additionally, we define certain structural properties on \acp{WDTA}
...
\begin{definition}
  ...
  with
  \begin{equation*}
    G_{i, j}((q', p'), d) = G_{i}(q', d)\cdot G_{j}(p', d)
  \end{equation*}
...
\end{definition}
Initially, we show that the induced measure of runs on
$\mathcal{A}_{1}\otimes\mathcal{A}_{2}$ coincides with the product measure of
individual runs on $\mathcal{A}_{1}$ and $\mathcal{A}_{2}$.
\begin{lemma}[Product measure of runs]
  ...
  $\mu_{r} = \mu_{r_{1}}\otimes\mu_{r_{2}}$.
\end{lemma}
\begin{proof}
\end{proof} 
