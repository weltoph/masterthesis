...
\begin{proof}
  ...
  \ref{thm:emptinesspospba}. By Theorem \ref{thm:pbatopra} and Theorem 
  \ref{thm:probautoequiv} there is for any \ac{PBA} an equivalent \ac{PRA}, 
  \ac{PPA} and \ac{PMA} which accept either with probability $0$ or $1$; thus, 
  positive and almost-sure acceptance coincide. Hence again computing a 
  strategy for the positive or almost-sure satisfaction of Rabin-, Parity- or 
  Muller-conditions for \acp{POMDP} solves the undecideable emptiness problem 
  for \acp{PBA} with positive acceptance.
\end{proof}
...
\begin{theorem}
  ...
  satisfies an associated Büchi-, Parity-, Rabin- or Muller-condition.
\end{theorem} 
\fxwarning{find a better reference for this and explain a little about 
endcomponents and such}
\begin{proof}
  All these conditions induce an $\omega$-regular language on the set of 
  states. Expressing this language by an \ac{NBA} allows us to use 
  \cite[Theorem 4.1.7.]{ComplexProbVerification} to solve these problems.
\end{proof}
...
\begin{definition}[Tree automaton]
  ...
  \begin{description}
    ...
    \item[Determinism] $\mathcal{A}$ is considered deterministic if for every
      $q\in Q$ and $\sigma\in\Sigma$ there is exactly one 
      $\tuple{q,\sigma,\tuple{q_{d}}_{d\in D}\in\Delta}$,
...
\end{definition}
We introduce as we are already accustomed fitting acronyms, e.g. \ac{DPTA}
\acuse{NPTA} \acuse{DRTA} \acuse{NRTA} \acuse{DMTA} \acuse{NMTA} \acuse{DBTA}
\acuse{NBTA}, and present an easy example to familiarize ourself with the 
notions of tree automata
\begin{example}
...
\end{example}
The difficulties to define a \ac{NBTA} to accept $\mathcal{L}_{\not\infty a}$ 
are inherent to the Büchi-condition. In fact it is possible to show that 
$\mathcal{L}_{\not\infty a}$ cannot be accepted by any \ac{NBTA}, thus 
...
\begin{theorem}
  ...
  \acp{NBTA} are strictly weaker than \ac{NMTA} in the sense that there exists 
  a language recognizable by a \ac{NMTA} but not recognizable by a \ac{NBTA}.
\end{theorem}
On the other hand, we can use already established technicalities, namely 
\acp{LAR}, to state the following
\begin{theorem}
  \cite[Theorem 8.7]{AutoLogInfGames}
  \acp{NMTA},\acp{NRTA} and \acp{NPTA} all recognize the same tree languages.
  \label{thm:treeautoequiv}
\end{theorem}
\begin{proof}
  This proof follows the same structure as the proof to Theorem 
  \ref{thm:omegaregularexp} in the sense that Parity- and Rabin-conditions can
  be expressed as Muller-conditions and Parity-conditions can be expressed as
  Rabin-conditions and any \ac{NMTA} can be transformed to an \ac{NPTA} as 
  follows. Considering an \ac{NMTA} $\mathcal{M} = \tuple{Q, q_{0}, D, \Sigma, 
  \Delta, \mathcal{F}}$ we define 
  $\mathcal{P} = \tuple{\lar(Q), \ell_{0}, D, \Sigma, \nabla, \parity}$
  (again $\ell_{0}$ is arbitrarily chosen) with
  \begin{equation*}
    \nabla = \set{
      \tuple{\ell_{q}, \sigma, \tuple{\up(\ell_{q}, q_{d})}_{d\in D}}:
      \tuple{q,\sigma,\tuple{q_{d}}_{d\in D}}\in\Delta
    }
  \end{equation*}
  and
  \begin{equation*}
    \parity(\interval{p_{1}\dots p_{n},h}) = \begin{cases}
      2\cdot h&\text{if }\set{p_{1},\dots, p_{n}}\in\mathcal{F},\\
      2\cdot h - 1&\text{otherwise}.
    \end{cases}
  \end{equation*}
  We obtain for every run in $\mathcal{M}$ a corresponding run in $\mathcal{P}$
  and vice versa and Lemma \ref{lem:larhitset} yields the acceptance 
  equivalence of these runs.
\end{proof}
We postpone the discussion of other aspects of tree automata, e.g. closure 
properties, after the introduction of another form of tree automata.
...
\begin{theorem}[The Simulation Theorem]
  ...
  There is an effective construction which, when given an \ac{AMTA} 
  on infinite trees, produces an equivalent \ac{NMTA}. Furthermore, given an 
  \ac{ABTA}, there is a way to effectively construct an equivalent \ac{NBTA}.
\end{theorem}

Again, we consider again the closure properties of alternating tree automata. 
Hereby, union and intersection are constructed very naturally, while negation 
takes some more effort.
\begin{proposition}
  The class of languages recognized by \acp{AMTA} is closed under union and
  intersection.
\end{proposition}
\begin{proof}
  Given two \acp{AMTA} 
  ...
  we construct an \ac{AMTA} for their union and one for their intersection.
  W.l.o.g. we assume that $Q\cap P = \emptyset$. The construction for the union 
  can be trivially obtained using the non-determinism by defining
  \begin{equation*}
    \mathcal{A}_{\cup} = \tuple{
      Q\cup P\uplus\set{q^{i}}, q^{i}, D, \Sigma, 
      \Delta_{1}\cup\Delta_{2}\cup\Delta_{\cup}, 
  ...
  \begin{equation*}
    \Delta_{\cup} = \set{
      \tuple{q^{i},\sigma,\delta_{A}}:
        \tuple{q_{0},\sigma,\delta_{A}}\in\Delta_{1}\text{ or }
        \tuple{p_{0},\sigma,\delta_{A}}\in\Delta_{2}
        \text{ for every }\sigma\in\Sigma
    }.
  \end{equation*}
  We can simply biject any run of $\mathcal{A}_{\cup}$ to one of the runs of
  $\mathcal{A}_{1}$ or $\mathcal{A}_{2}$ by mapping $q^{i}$ to $q_{0}$ or 
  $p_{0}$ respectively.

  For the intersection it suffices to initially dispatch one transition of each
  automaton which independently traverse the tree. Considering the possible 
  runs of a \ac{PBA} on a word as tree with weighted transitions then the same
  idea is used in the proof to Theorem \ref{thm:pbaboolalgebra} for the union
  operator. Hence, we define
  \begin{equation*}
  ...
  \end{equation*}
  Thus, we can construct from the resulting run in $\mathcal{A}_{\cap}$ two
  individual runs in $\mathcal{A}_{1}$ and $\mathcal{A}_{2}$ respectively.
\end{proof}
We consider negation after introducing graph games which allows us to define
a negation operation very naturally.
