...
\begin{definition}[Emptiness Game]
  ...
  \begin{equation*}
    A = \Sigma\times\tuple{Q\rightarrow\Delta_{\sigma}}
  \end{equation*}
  where $\tuple{Q\rightarrow\Delta_{\sigma}}$ is the set of functions that map
  a state $q\in Q$ to any generator in $\Delta(q,\sigma)$. The emptiness game
  is then defined as \ac{POMDP}
  \begin{equation*}
...
\fxfatal{this allows for postive acceptance to be tricked by using move to
bottom to complete the automaton; can be fixed by removing q,sigma states
and encode this decision already in the action in q,d}

\section{Equivalence of \acp*{WDTA} and \acp*{PBA}}
By reducing the directions to a singelton set a tree degenerates to a single
path. This path can be interpreted as one word and thus, we can identify any
tree $t: D = \set{0}\rightarrow\Sigma$ as a function
$t':\mathbb{N}\rightarrow\Sigma$ with $t'(n) = t(0^{n})$ and as described in
Example \ref{ex:pba} we obtain from $t'$ a word in $\Sigma^{\omega}$ (and vice
versa). This gives reason to incorporate \acp{PBA} into the theory of
\acp{WDTA} by the following theorem
\begin{theorem}
...
\end{theorem}
\fxfatal{add proofs and lemmas}
\fxfatal{derive undecideable results for Streett-WDTA with qualitative
acceptance from 0/1 Streett PBA equivalent to >0- PBA}
