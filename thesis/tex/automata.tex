...
\begin{proof}
  ...
  \end{align*}
\end{proof}
The nature of $f_{\mathcal{A}}$ allows for further refinement, namely by
examining the probability of a certain state sequence rather than if this
sequence is in $T$ or not. Thus, we define
\begin{equation*}
  s:\Runs\times\Paths\times Q^{\omega}\rightarrow\interval{0,1}
  \text{ with }
  s(r,p,w) = \begin{cases}
    1&\text{if }r(p) = w\\
    0&\text{otherwise.}
  \end{cases}
\end{equation*}
Which allows for a reformulation of $f_{\mathcal{A}}$ as
\begin{equation*}
  f_{\mathcal{A}}(r,p) = \sum\limits_{w\in T}s(r,p,w).
\end{equation*}
And therefore by Proposition \ref{prop:pwabyf} we state
\begin{proposition}
  For a \ac{PWA} $\mathcal{A}$ and a tree $t$ holds
  \begin{equation*}
    \mathcal{A}\text{ accepts }t
    \text{ iff }
    \int \sum\limits_{w\in T}s d\mu_{t}\otimes\mu_{B} = 1.
  \end{equation*}
  \label{prop:pwabys}
\end{proposition}
...
\begin{proof}
  ...
  $\tuple{Q\times D}$ which yields
  \begin{align*}
    \mu_{r}(\cyl&(\tuple{q_{1},d_{1}}\dots\tuple{q_{n},d_{n}}))\\
      &=\prod\limits_{1\leq i<n} B(d_{i})\sum\limits^{
        \delta\in\supp(\Delta(q_{i},t(d_{1}\dots d_{i})))}_{\delta(d_{i+1}) = q_{i+1}}\Delta(\delta)\\
      &=\prod\limits_{1\leq i<n} B(d_{i})\cdot\prod\limits_{1\leq i<n}
        \sum\limits^{\delta\in\supp(\Delta(q_{i},t(d_{1}\dots d_{i})))}_{\delta(d_{i+1}) = q_{i+1}}
        \Delta(\delta)\\
      &=\mu_{B}(\cyl(d_{1}\dots d_{n}))\cdot\prod\limits_{1\leq i<n}
        \sum\limits^{\delta\in\supp(\Delta(q_{i},t(d_{1}\dots d_{i})))}_{\delta(d_{i+1}) = q_{i+1}}
        \Delta(\delta)\\
      &=\mu_{B}(\cyl(d_{1}\dots d_{n}))\cdot\sum_{w\in\cyl(q_{0}q_{1}\dots q_{n})}\int\limits_{p\in\cyl(d_{1}\dots d_{n})}
        \int\limits_{r\in\Runs}s(r,p,w)d\mu_{t}d\mu_{B}\\
  \end{align*}
\end{proof}
