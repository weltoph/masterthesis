...
\begin{example}
  ...
  If $\lambda$ is set to $\frac{1}{2}$, this language is - to a certain degree 
  - dual to the language in Example \ref{ex:posacceptingpba} (without 
  considering all words ending in $a^{\omega}$ or those words starting in $b$ 
  or ommitting an \enquote{$a$-phase}). This duality is rooted in structural 
  similarities. Examining the \emph{Separation} part, it mirrors the 
  $a$-transitions of the \ac{PBA} in Figure \ref{fig:posacceptingpba}. But an 
  occurences of $b$ does not discard the probability mass in $q_{2}$ ($q_0$, 
  respectively) but saves it in an accepting state and re-introduces into the 
  circulation (as seen in the \emph{Disposal} part).
...
\begin{definition}[Tree automaton]
  ...
  \begin{description}
    ...
    \item[Determinism] $\mathcal{A}$ is considered deterministic if for every
      ...
      $\tuple{q,\sigma,\tuple{q_{d}}_{d\in D}\in\Delta}$. We then refer to it 
      as \ac{DTA} otherwise as \ac{NTA},
...
\end{definition}
We present a very simple introductory example to familiarize us with the 
notions of \acp{NTA}:
\begin{example}
...
\end{example}
The difficulties to define a \ac{NTA} with a Büchi-condition to accept 
$\mathcal{L}_{\not\infty a}$ are inherent to the Büchi-condition for tree 
automata. In fact it is possible to show that $\mathcal{L}_{\not\infty a}$ 
cannot be accepted by any \ac{NTA} with Büchi-condition, thus separating the 
expressiveness of tree automata as in
\begin{theorem}
  ...
  sense that there exists a language recognizable by a \ac{NTA} with a 
  Muller-condition but not recognizable by a \ac{NTA} with a Büchi-condition.
\end{theorem}
\begin{proof}
  We omit the proof since it requires some technicalities which are well
  documented but not useful for the further course of this thesis.
\end{proof}
...
\begin{theorem}
  ...
  \acp{NTA} with Muller-, Parity- and Rabin-conditions all recognize the same
  tree languages.
\end{theorem}
\begin{proof}
  ...
  Rabin-conditions and any \ac{NTA} with Muller-conditions can be transformed 
  to an \ac{NTA} with Parity-condition as follows. Considering an \ac{NTA} 
  $\mathcal{M} = \tuple{Q, q_{0}, \Delta}$ with an associated Muller-condition
  $\mathcal{F}$ we define $\mathcal{P} = \tuple{\lar(Q), \ell_{0}, \nabla}$
...
\begin{example}
  ...
  \enquote{searching} part of the run more concisely\footnote{
    Occasionally, e.g. in \cite[Chapter 9]{AutoLogInfGames}, \acp{ATA} are 
    equipped with the possibility to render a subtree accepted regardless of 
    its structure. We explicitly do not include this notion, but it is easy to 
    see that this would allow to equivalently define transitions
...
\fxfatal{run epsilon not yet consistent}
\begin{definition}[Alternating Tree Automata]
  ...
  relation $\Delta$ we can define an \ac{ATA} $\mathcal{A}$ over $D$-ary 
  $\Sigma$-trees as
  \begin{equation*}
    \mathcal{A} = \tuple{Q, q_{0}, D, \Sigma, \Delta, \Acc\subseteq Q^{\omega}}.
  \end{equation*}
  The semantics of $\mathcal{A}$ is given by a run $r$ of $\mathcal{A}$ on a
  tree $t$. Here $r$ is a $D\times Q$-ary $\nabla$-tree where $\nabla$ is the
  set of all clause-functions occuring in $\Delta$ with the following
  necessities:
  \begin{enumerate}
  ...
  M_{r} = \set{\tuple{d_{1}, q_{1}}\tuple{d_{2}, q_{2}}\dots\in
    ...
    \tuple{d_{i}, q_{i}})(d_{i+1}, q_{i+1}) = 1 \text{ for }i>0}
  \end{equation*}
  and require that
  \begin{equation*}
