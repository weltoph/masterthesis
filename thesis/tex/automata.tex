...
Building upon \cite{RandAutoInfTrees} we expand upon the notion of weighting
...
\begin{lemma}[Measurability]
  The set ${\alpha = \tuple{q_{1},d_{1}}\tuple{q_{2},d_{2}}\dots\in
  (Q\times D)^{\omega}\mid q_{0}q_{1}q_{2}\dots\in T_{\mathcal{A}}$ is
  measurable in the $\sigma$-algebra that is induced by the ring of cylinders.
\end{lemma}
\begin{proof}
  We start by the co-Büchi condition which is the dual condition to the Büchi
  condition.
\end{proof}
We say that $\mathcal{A}$ accepts $t$ if such an
accepting run exists. Furthermore we define certain structural properties on
\acp{WDTA}
\begin{definition}[Structural Properties]
  ...
  \begin{description}
    ...
    \item [uniformly distributed (u.d.)] if $\mathcal{A}$ is uni-directional
      ...
      $G_{1}(q, d) = G_{2}(p, d)$ (for the unique $p, q$ for which
      $G_{1}(q, d) > 0$, respectively $G_{2}(p, d) > 0$).
  \end{description}
\end{definition}
