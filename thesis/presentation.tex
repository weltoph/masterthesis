...
\begin{document}
  ...
  \begin{frame}
    ...
    \begin{tabular}{llm{0.5\textwidth}}
      choiceless (c.l.): & $\substack{\size{\set{G: \tuple{q, \sigma, G}\in\Delta}}
        = 1\\\text{ for all }q\in Q, \sigma\in\Sigma}$ & \\
      uni-directional: & $\substack{\text{for every }
    ...
    \begin{columns}
      ...
      \begin{column}{0.65\textwidth}
        \begin{overlayarea}{\textwidth}{\textheight}
          \begin{onlyenv}<3->
            \begin{itemize}
              \item<4-> $Q = S$ with $q_{0} = s_{0}$
              \item<5-> $D = \interval{S}_{\sim}$ and $\Sigma = A$
              \item<6-> $G_{a}^{s}(s', d) = \begin{cases}
